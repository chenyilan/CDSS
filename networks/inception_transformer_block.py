...
class InceptionTransformer(nn.Module):
    def __init__(self,  patch_size=16, in_chans=3,  embed_dims=None, depths=None,
                 num_heads=None, num_patches=None,mlp_ratio=4., qkv_bias=True, 
        ...
        self.patch_embed = embed_layer(kernel_size=3, stride=2, padding=1, in_chans=in_chans, embed_dim=embed_dims)
        self.num_patches = num_patches
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, num_patches, embed_dims))
        self.blocks = nn.Sequential(*[
            Block(
                dim=embed_dims, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, norm_layer=norm_layer, act_layer=act_layer, attention_head=attention_heads[i], pool_size=2,)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 3, 1)
        B, H, W, C = x.shape
        x = x+ self._get_pos_embed(self.pos_embed, self.num_patches, H, W)
        x = self.blocks(x)
        x = x.permute(0, 3, 1, 2)
...
if __name__ == '__main__':
    ...
    # x = Variable(torch.FloatTensor(np.random.random((1, 950, 64, 64))),requires_grad = True).to(device)
    img = torch.randn((2, 63, 128, 128)).to(device)
    depths = [3, 3, 9, 3]
    embed_dims = [96, 192, 320, 384]
    num_heads = [3, 6, 10, 12]
    attention_heads = [1] * 3 + [3] * 3 + [7] * 4 + [9] * 5 + [11] * 3
    ...
    model = InceptionTransformer(depths=depths[0],
                                embed_dims=63,
                                in_chans = 63,
                                num_heads=num_heads[0],
                                num_patches=num_patches,
                                attention_heads=attention_heads[0:3],
                                num_classes=0,
                                use_layer_scale=False, layer_scale_init_value=1e-6).to(device)
